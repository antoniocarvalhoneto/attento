...
function Catalog({ kind, data, refresh }: { kind: Kind; data: Collections; refresh(): void }) {
  ...
  return <><PageHead title={rooms ? 'Salas' : 'Profissionais'} description={rooms ? 'Consulte a capacidade e a situação de cada sala.' : 'Vincule cada profissional à unidade, à sala e ao turno de atendimento.'}><button className="btn btn-primary" onClick={create} disabled={!data.units.length || (!rooms && !data.rooms.length)}>{label}</button></PageHead>
    {!rooms && !data.rooms.length && <p role="status">Cadastre uma sala antes de adicionar um profissional.</p>}
    {rooms && <label><input type="checkbox" checked={showArchived} onChange={event => setShowArchived(event.target.checked)} /> Mostrar salas anteriores</label>}
    <DataTable empty={rooms ? 'Nenhuma sala cadastrada. Use “Nova sala” para incluir a primeira.' : 'Nenhum profissional cadastrado.'} headers={rooms ? ['Sala', 'Unidade', 'Área / capacidade', 'Status', 'Ações'] : ['Nome', 'Especialidade', 'Unidade', 'Sala', 'Turno', 'Valor', 'Status', 'Ações']} rows={data[kind].filter(item => !('archived' in item && item.archived) || showArchived).map(item => ({ id: item.id, cells: [item.name, ...('capacity' in item ? [nameOf(data.units, item.unitId), item.areaM2 ? `${item.areaM2} m²` : item.capacity ?? 'Não informada'] : [item.specialty, nameOf(data.units, item.unitId), nameOf(data.rooms, item.roomId), item.shift, money(item.value)]), <Status key="status" value={item.status} />, <RecordActions key="actions" onView={() => setViewing(item)} onEdit={() => setEditing(item)} onDelete={() => setRemoving(item.id)} />] }))} />
    ...
    {viewing && <Modal title={viewing.name} onClose={() => setViewing(null)}><div className="modal-body"><div className="info-row"><span>Unidade</span><span>{nameOf(data.units, viewing.unitId)}</span></div>{'capacity' in viewing ? <><div className="info-row"><span>Capacidade</span><span>{viewing.capacity ? viewing.capacity + ' pessoas' : 'Não informada'}</span></div>{viewing.areaM2 && <p>Área: {viewing.areaM2} m²</p>}{viewing.shiftPrices && <p>Referência dos turnos fixos: manhã {money(viewing.shiftPrices.morning)} · tarde {money(viewing.shiftPrices.afternoon)} · noite {money(viewing.shiftPrices.night)}. Valores não são preços por hora.</p>}<p>{viewing.description || 'Sem descrição.'}</p></> : <><p>Especialidade: {viewing.specialty || '—'}</p><p>Sala: {nameOf(data.rooms, viewing.roomId)}</p><p>Turno: {viewing.shift}</p><p>Valor: {money(viewing.value)}</p></>}<Status value={viewing.status} /></div></Modal>}
  </>
}
function CatalogEditor({ kind, record, data, onClose, onSaved }: { kind: Kind; record: Record; data: Collections; onClose(): void; onSaved(): void }) {
  ...
  return <Modal title={record.id ? kind === 'rooms' ? 'Editar sala' : 'Editar profissional' : kind === 'rooms' ? 'Nova sala' : 'Adicionar profissional'} onClose={onClose}><form onSubmit={event => { event.preventDefault(); try { if ('capacity' in draft) saveRecord('rooms', { ...draft, name: draft.name.trim() }); else saveRecord('professionals', { ...draft, name: draft.name.trim() }); onSaved() } catch (failure) { setError((failure as Error).message) } }}><div className="modal-body">
    ...
    {'capacity' in draft ? <><Field label="Descrição" value={draft.description} onChange={event => update({ description: event.target.value })} /><Field label="Capacidade" type="number" min="1" step="1" value={draft.capacity ?? ''} onChange={event => update({ capacity: event.target.value ? Number(event.target.value) : null })} /><Select label="Status" value={draft.status} onChange={status => update({ status })}><option value="disponivel">Disponível</option><option value="manutencao">Manutenção</option><option value="inativa">Inativa</option></Select></> : <>
      <Field label="Especialidade" value={draft.specialty} onChange={event => update({ specialty: event.target.value })} /><Select label="Sala" value={draft.roomId} required onChange={roomId => update({ roomId })}><option value="">Selecione</option><Options items={data.rooms.filter(room => room.unitId === draft.unitId)} /></Select><Select label="Turno" value={draft.shift} onChange={shift => update({ shift })}>{['Manhã', 'Tarde', 'Noite', 'Conforme grade fixa'].map(shift => <option key={shift}>{shift}</option>)}</Select><Field label="Valor (R$)" type="number" min="0" step="0.01" required value={draft.value} onChange={event => update({ value: Number(event.target.value) })} /><Select label="Status" value={draft.status} onChange={status => update({ status })}><option value="ativo">Ativo</option><option value="inativo">Inativo</option></Select>
