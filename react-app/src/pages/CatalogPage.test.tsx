...
test('edita e exclui sala sem vínculos, mas preserva sala vinculada', async () => {
  ...
  const linked = screen.getByText('Sala 01').closest('tr')!
...
test('profissional é cadastrado com unidade e sala escolhidas', async () => {
  ...
  await userEvent.selectOptions(screen.getByLabelText('Unidade'), 'unit_sul')
  expect(screen.getByLabelText('Sala')).toHaveValue('room_4')
  await userEvent.click(screen.getByRole('button', { name: 'Salvar' }))
  expect(readData().professionals.find(item => item.name === 'Profissional React')).toMatchObject({ unitId: 'unit_sul', roomId: 'room_4' })
})
