...
function InsuranceList({ data, refresh }: { data: Collections; refresh(): void }) {
  ...
  return <><PageHead title="Convênios" description="Registre a quantidade de atendimentos e o valor de cada repasse."><button className="btn btn-primary" disabled={!data.professionals.length} onClick={() => setEditing({ id: '', insuranceType: INSURANCE_TYPES[0], professionalId: data.professionals[0]?.id || '', quantity: 1, unitValue: 0, status: 'pendente' })}>Adicionar registro</button></PageHead>{!data.professionals.length && <p>Cadastre um profissional antes de adicionar um registro.</p>}{error && <p role="alert">{error}</p>}
