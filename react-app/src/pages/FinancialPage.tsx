...
function Financial({ data, refresh }: { data: Collections; refresh(): void }) {
  ...
  return <><PageHead title="Contas / Financeiro" description="Confira vencimentos, registre pagamentos e exporte o relatório dos filtros selecionados."><button className="btn btn-secondary" onClick={exportReport}>Exportar CSV</button> <button className="btn btn-primary" disabled={!data.professionals.length} onClick={() => setEditing({ id: '', professionalId: data.professionals[0]?.id || '', description: '', value: 0, dueDate: dateKey(), status: 'pendente', paymentDate: null })}>Nova conta</button></PageHead>
    {!data.professionals.length && <p>Cadastre um profissional antes de adicionar uma conta.</p>}{error && <p role="alert">{error}</p>}
    ...
    <div className="section stat-grid"><StatCard label="Total geral" value={money(total())} /><StatCard label="Total recebido" value={money(total('pago'))} /><StatCard label="Total pendente" value={money(total('pendente'))} warning={total('pendente') > 0} /><StatCard label="Total vencido" value={money(total('vencido'))} warning={total('vencido') > 0} /></div>
    <DataTable empty={data.financial.length ? 'Nenhuma conta corresponde aos filtros. Altere a seleção ou use “Limpar”.' : 'Nenhuma conta cadastrada. Use “Nova conta” para registrar um valor e seu vencimento.'} headers={['Profissional', 'Descrição', 'Valor', 'Vencimento', 'Status', 'Pagamento', 'Ações']} rows={list.map(item => ({ id: item.id, cells: [nameOf(data.professionals, item.professionalId), item.description, money(item.value), displayDate(item.dueDate), <Status key="status" value={item.status} />, displayDate(item.paymentDate || ''), <RecordActions key="actions" onEdit={() => setEditing(item)} onDelete={() => setRemoving(item.id)} onPay={item.status !== 'pago' ? () => pay(item.id) : undefined} />] }))} />
