...
function Financial({ data, refresh, reports }: { data: Collections; refresh(): void; reports: boolean }) {
  ...
  return <><PageHead title={reports ? 'Relatórios Financeiros' : 'Contas / Financeiro'} description={reports ? 'Selecione o período e o profissional para exportar as contas.' : 'Confira vencimentos e registre os pagamentos recebidos.'}>{reports ? <button className="btn btn-primary" onClick={exportReport}>Exportar CSV</button> : <button className="btn btn-primary" disabled={!data.professionals.length} onClick={() => setEditing({ id: '', professionalId: data.professionals[0]?.id || '', description: '', value: 0, dueDate: dateKey(), status: 'pendente', paymentDate: null })}>Nova conta</button>}</PageHead>
