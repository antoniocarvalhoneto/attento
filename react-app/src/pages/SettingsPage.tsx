...
export function SettingsPage(props: Props) { return <Page>{() => <Settings {...props} />}</Page> }
function Settings({ user, theme, onThemeChange }: Props) {
  const [whatsapp, setWhatsapp] = useState(() => readSettings().whatsapp || ''), [error, setError] = useState(''), [message, setMessage] = useState('')
