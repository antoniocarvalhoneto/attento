...
function Availability({ data, refresh, user }: { data: PageData; refresh(): void; user: User }) {
  ...
  return <>
    ...
    {rooms.map(room => <section className="section" key={room.id}><div className="section-head"><h2>{room.name} <span className="text-muted">— {nameOf(data.units, room.unitId)}</span></h2></div><div className="agenda-wrap" role="region" aria-label={`Agenda de ${room.name}`} tabIndex={0}><div className="agenda-grid"><div className="agenda-corner" />{WEEK_DAYS.map((day, index) => <div className="agenda-head" key={day}>{day}<br /><small>{displayDate(slotDate(Number(week), index))}</small></div>)}
